...
- **Link Extraction & Domain Graph Analysis** - Extracts links and builds an Adjacency-list based inter-domain link graph.
...
You can build the `crawler` and `benchmark_runner` executables using the provided script:
```bash
./build.sh
```
...
### Running the Crawler
```bash
./build/crawler <seed_url> <max_pages> <num_threads>
```
*Example:*
```bash
./build/crawler http://localhost:8080/ 200 4
```
...
The crawler generates rich data files in the current directory:
